...
from .melody_model import Melody
...
class CLAP(nn.Module):
    def __init__(
        ...
        # melody branch
        self.melody_branch = Melody()
        self.melody_mlp_1024_to_768 = nn.Linear(768, 768)
        self.melody_projection = nn.Sequential(
            nn.Linear(768, self.joint_embed_shape),
            mlp_act_layer,
            nn.Linear(self.joint_embed_shape, self.joint_embed_shape),
        )
        self.melody_transform = MLPLayers(
            units=[
    ...
    def encode_melody(self, melody_text, device):
        return self.melody_branch(melody_text, device)
